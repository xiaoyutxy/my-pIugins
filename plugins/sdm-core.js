...
try {
    ...
    const SDM_SIG = '@@SDM_PLUGIN_ID:sdm-core@@';
    ...
    const _sdmCheckUpdate = async () => {
        ...
        try {
            ...
            for (const src of srcList) {
                ...
                const vrf = await _sdmRun(`_s=$(wc -c < ${_sdmSq(SDM_PENDING_JS)} 2>/dev/null || echo 0); _c=$(grep -c 'SDM_PLUGIN''_ID:sdm-core' ${_sdmSq(SDM_PENDING_JS)} 2>/dev/null || echo 0); _v=$(grep -o "const PLUGIN_VERSION = '[^']*'" ${_sdmSq(SDM_PENDING_JS)} 2>/dev/null | head -1 | sed "s/.*'\\(.*\\)'.*/\\1/"); echo "$_s|$_c|$_v"`, 10000);
