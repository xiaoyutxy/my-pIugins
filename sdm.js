...
/* ✨💫 更新按钮旁的小星星：常驻闪烁（opacity 最低 .25，只明暗闪动，不会整个消失） */
...
/* ✨💫 星星常驻显示并闪烁（不能设 opacity:0，否则大半时间看不见） */
...
/* 有更新时闪得更急促，配合脉冲一起吸引注意 */
.sdm-check-btn.has-update .sdm-deco-1{animation-duration:1.2s;animation-delay:0s}
.sdm-check-btn.has-update .sdm-deco-2{animation-duration:1.2s;animation-delay:.6s}
...
/* 注：樱花雨（#smart_sakura_container / #smart_sakura_canvas / .smart_sakura）已整体移除 */
...
/* 注：卡片扫光 ::after 已移除（用户要求删「扫光」） */
/* 注：卡片悬停上下弹跳已移除，保留卡片本身 */

/* ── ✨ 二次元发光标题（仅保留静态光晕，摇摆与 ✨🎀 装饰动画已移除） ── */
...
/* 注：按钮扫光 ::after 已移除；悬停弹跳已移除，只保留放大反馈 */
...
/* 注：樱花开关按钮 #smart_sakura_toggle 已随樱花雨一并移除 */
...
/* 注：星光闪粉层 .sdm2-sparkles 已彻底移除 —— 每张卡片动态生成 5~8 个带 drop-shadow
   的 <i> 粒子且跑两轮，是列表滚动卡顿的主因 */
...
/* ── 🌸 顶部标题装饰行 ── */
/* 注：彩虹下划线 .sdm2-title-underline 已移除 */
...
/* 关键点：作用域限定在 #IFRAME_SMART 内，绝不写 html.sdm-no-fx * 这种全局通配，否则会误伤其他插件 */
/* 更新按钮整棵子树用 :not() 排除，压根不参与压缩，所以关特效时也照常动 */
html.sdm-no-fx #IFRAME_SMART,
html.sdm-no-fx #IFRAME_SMART *:not(.sdm-check-btn):not(.sdm-btn-icon-wrap):not(.sdm-btn-icon):not(.sdm-deco-1):not(.sdm-deco-2):not(.sdm-update-tip):not(.sdm-update-badge),
html.sdm-no-fx #IFRAME_SMART *:not(.sdm-check-btn):not(.sdm-btn-icon-wrap):not(.sdm-btn-icon):not(.sdm-deco-1):not(.sdm-deco-2):not(.sdm-update-tip):not(.sdm-update-badge)::before,
html.sdm-no-fx #IFRAME_SMART *:not(.sdm-check-btn):not(.sdm-btn-icon-wrap):not(.sdm-btn-icon):not(.sdm-deco-1):not(.sdm-deco-2):not(.sdm-update-tip):not(.sdm-update-badge)::after{animation-duration:1ms !important;animation-delay:0s !important;animation-iteration-count:1 !important;transition-duration:1ms !important;transition-delay:0s !important}
...
<div id="IFRAME_SMART" style="width:100%;margin-top:10px;">
    ...
    ;(function sdm2Ambience() {
        ...
        // 2. （已移除）星光闪粉层 sparkCards —— 每张卡片要动态生成 5~8 个带 drop-shadow
        //    的粒子，且执行两轮，是列表滚动卡顿的主因。对应 CSS 与粒子动画的关键帧
        //    定义也已一并删除。
    ...
    /* ============ 🚦 全局动效总开关 ============ */
    // 关闭后：本插件（#IFRAME_SMART）内的 CSS 动画/过渡瞬间结束（1ms）不再持续重绘。
    // 注意：只作用于本插件容器及其后代，其他插件的动画不受影响。
    // 更新按钮（.sdm-check-btn）整棵子树被 :not() 排除，始终保持动效。
    var _fxOn = true
    try {
    ...
    ;(async () => {
        ...
        var _showLimitModal = async function() {
            ...
            for (var i = 0; i < devices.length; i++) {
                ...
                deviceListHtml += '<div style="flex:1;min-width:0;">'
                deviceListHtml += '<div style="font-size:.72rem;color:#e2e8f0;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + name + '</div>'
                deviceListHtml += '<div style="font-size:.58rem;color:#a78bfa;font-family:monospace;margin-top:2px;">IP: ' + _esc(d.ip) + '</div>'
                deviceListHtml += '</div>'
